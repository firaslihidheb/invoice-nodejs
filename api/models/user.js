...
class User {
    ...
    updateUser(req, res) {
        ...
        try {
           ...
           // assert(req.body.last_name, 'Last name is required');
            ...
            assert(req.body.qualification, 'Qualification is required');
            ...
            connectionPool.query(`UPDATE user SET name = ?, last_name = ?,  password = ?, qualification = ?, 
        email = ?,type_id = ? WHERE id = ? `, [
                name, last_name, hashedPassword, qualification,  email, type_id,id], function (error, result, fields) {
                if (error) {
                    res.status(500).send({
                        status: "error",
                        code: error.code,
                    ...
                    res.status(500).send({
                        status: "error",
                        message: "A user with same last name number already exists"
                    });
                }
            });
        } catch (e) {
    ...
    addUser(req, res) {
        ...
        try {
            ...
            connectionPool.query(`INSERT IGNORE into user(name, last_name, password, qualification, 
        email,type_id) VALUES(?,?,?,?,?,?)`, [
                name, last_name, hashedPassword, qualification,  email, type_id], function (error, result, fields) {
                if (error) {
                    res.status(500).send({
                        status: "error",
                        code: error.code,
    ...
    loginUser(req, res) {
        try {
            ...
            connectionPool.query(`SELECT id,password FROM user where name = ?`, username,
                    ...
                    if (error) {
                        ...
                        if (result.length > 0) {
                            ...
                            if (compareSync(password, result[0].password)) {
                                ...
                                connectionPool.query(`UPDATE user SET auth_token = ? , expiry_time = (SELECT DATE_ADD(NOW(), INTERVAL 24 HOUR)) WHERE id = ?`, [token, result[0].id], function (error2, result2, fields2) {
                                    if (error) {
                                        res.status(500).send({
                                            status: "error",
                                            code: error.code,
     ...
     */
    logoutUser(req, res) {
        connectionPool.query(`UPDATE user SET auth_token = ? WHERE id = ?`, ["", req.user_id], function (error2, result2, fields2) {
            if (error2) {
                res.status(500).send({
                    status: "error",
                    code: error2.code,
    ...
    authenticateToken(req, res, next) {
        ...
        try {
            connectionPool.query(`SELECT id, expiry_time FROM user where auth_token = ?;`, token, function(error, result, fields) {
                if (error) {
                    res.status(500).send({
                        status: "error",
                        code: error.code,
    ...
    getUsers(req, res){
            ...
            connectionPool.query(`SELECT * FROM user`, 
                function(error, result, fields) {
                    if (error) {
                        res.status(500).send({
                            status: "error",
