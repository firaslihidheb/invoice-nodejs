...
class User {
    ...
    try {
     ...
     // );
      ...
      connectionPool.query(
        `UPDATE user SET name = ?, last_name = ?, qualification = ?, 
        email = ?,type_id = ? WHERE id = ? `,
        [name, last_name, qualification, email, type_id, id],
  ...
  getUser(req, res) {
    try {
        assert(req.params.user_id, 'Customer Id not provided');

        connectionPool.query(`SELECT * FROM user where id = ? `,
            [req.params.user_id], function (error, result, fields) {
                if (error) {
                    res.status(500).send({
                        status: "error",
                        code: error.code,
  ...
  addUser(req, res) {
    try {
      ...
      connectionPool.query(
        `INSERT  into user(name, last_name, password, qualification, 
        email,type_id) VALUES(?,?,?,?,?,?)`,
        [name, last_name, hashedPassword, qualification, email, type_id],
        function (error, result, fields) {
          if (error) {
            res.status(500).send({
              status: "error",
          ...
          if (error) {
            ...
            if (result.length > 0) {
              ...
              if (compareSync(password, result[0].password)) {
                ...
                connectionPool.query(
                  `UPDATE user SET auth_token = ? , expiry_time = (SELECT DATE_ADD(NOW(), INTERVAL 24 HOUR)) WHERE id = ?`,
                  [token, result[0].id],
  ...
  authenticateToken(req, res, next) {
    ...
    try {
      connectionPool.query(
        `SELECT id, expiry_time FROM user where auth_token = ?;`,
        token,
        function (error, result, fields) {
          if (error) {
            res.status(500).send({
